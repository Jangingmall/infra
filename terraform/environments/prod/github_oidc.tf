...
#
# 🔴 GitHub 리포지토리 Settings → Environments → "production"에
...
module "github_oidc" {
  ...
  github_repo    = "infra"
  github_subject = "environment:production"
...
# ---- Backend 팀 CI (ECR push, 모든 브랜치/환경 허용) ----
  ...
  policy = jsonencode({
    ...
    Statement = [
      { Effect = "Allow", Action = "ecr:GetAuthorizationToken", Resource = "*" },
      {
        ...
        Resource = "arn:aws:ecr:ap-northeast-2:750240012008:repository/jangin-app"
      }
    ]
  })
}
...
# ---- GenAI 팀 CI (ECR push, 모든 브랜치/환경 허용) ----
  ...
  policy = jsonencode({
    ...
    Statement = [
      { Effect = "Allow", Action = "ecr:GetAuthorizationToken", Resource = "*" },
      {
        ...
        Action = ["ecr:BatchCheckLayerAvailability", "ecr:PutImage", "ecr:InitiateLayerUpload", "ecr:UploadLayerPart", "ecr:CompleteLayerUpload"]
        Resource = [
          "arn:aws:ecr:ap-northeast-2:750240012008:repository/jangin-ai/sglang",
          "arn:aws:ecr:ap-northeast-2:750240012008:repository/jangin-ai/ollama",
        ]
      }
    ]
  })
