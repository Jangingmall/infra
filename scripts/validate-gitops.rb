...
%w[stage prod].each do |environment|
  ...
  expected_names = %w[workloads cloudnative-pg argo-rollouts secrets-store-csi backend-networking cert-manager barman-cloud cnpg-backup aws-load-balancer-controller metrics-server nvidia-device-plugin].map { |n| "#{environment}-#{n}" } +
...
ai_api = ai['containers'].find { |container| container['name']=='ai-ollama' }
check(ai_api && ai_api['env'].any? { |e| e['name']=='DB_PASSWORD_FILE' && e['value']=='/mnt/secrets-store/password' }, 'AI password file contract missing')
check(ai_api['volumeMounts'].any? { |v| v['name']=='ai-chatbot-secrets' && v['readOnly']==true }, 'AI CSI mount missing')
